...
echo Port configuré : %AUTH_SERVICE_PORT%
if not "%AUTH_SERVICE_PORT%"=="8081" (
    echo Attention : Le port n'est pas 8081 comme attendu !
    echo Port actuel : %AUTH_SERVICE_PORT%
)

set SERVER_PORT=%AUTH_SERVICE_PORT%
...
if errorlevel 1 (
    echo Erreur lors de l'installation du service d'authentification
    exit /b 1
)
